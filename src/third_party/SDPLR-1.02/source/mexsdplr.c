...
extern void dsyrk_();
int classifyApq(int*, char*, int*, int*, int, int, int, int*, int*, int*, int*);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  */
  ...
  /* correct # of outputs */
  if(nlhs > 4)
    mexErrMsgTxt("sdplr: At most four output arguments, please.");

  /* correct # of inputs */
  if(nrhs < 4 || nrhs > 6)
    mexErrMsgTxt("sdplr: Four to six arguments, please.");
  ...
  if( (mxIsSparse(A_) == 1 && mxGetPi(A_) != NULL) ||
    ...
    mexErrMsgTxt("sdplr: Input 1 must be of type double, please.");

  /* matrix passed in for b (not complex) */
  if( (mxIsSparse(b_) == 1 && mxGetPi(b_) != NULL) ||
      (mxIsSparse(b_) == 0 && (mxIsDouble(b_) == 0 || mxIsComplex(b_) == 1)) )
    mexErrMsgTxt("sdplr: Input 2 must be of type double, please.");

  /* b is a col vec */
  if( mymin( mxGetM(b_) , mxGetN(b_) ) != 1)
    mexErrMsgTxt("sdplr: Input 2 must be a vector, please.");
  ...
  if( (mxIsSparse(c_) == 1 && mxGetPi(c_) != NULL) ||
    ...
    mexErrMsgTxt("sdplr: Input 3 must be of type double, please.");
  ...
  if( mymin( mxGetM(c_) , mxGetN(c_) ) == 0 )
    ...
    if( mymin( mxGetM(c_) , mxGetN(c_) ) != 1 )
      mexErrMsgTxt("sdplr: Input 3 must be a vector, please.");
    c_empty = 0;
  }
  ...
  if(nrhs > 3) {
    ...
    if(mxIsStruct(K_) == 0)
      mexErrMsgTxt("sdplr: Input 4 must be of type struct, please.");
      ...
      if( mxGetM(field)*mxGetN(field) != 0 ) {
        ...
        if(mat[0] != 0.0) {
          ...
          if(strcmp(fieldname, "s") != 0 && strcmp(fieldname, "l") != 0)
            mexErrMsgTxt("sdplr: Sorry, only linear and semidefinite cones supported.");
    ...
    if(Kl_ != NULL)
      if(mxIsDouble(Kl_) == 0 || mxIsComplex(Kl_) == 1)
        mexErrMsgTxt("sdplr: Linear portion of cone must be of type double, please.");
    if(Ks_ != NULL)
      if(mxIsDouble(Ks_) == 0 || mxIsComplex(Ks_) == 1)
        mexErrMsgTxt("sdplr: Semidefinite portion of cone must be of type double, please.");
    ...
    if(nrhs > 4) {
      ...
      if(mxIsStruct(pars_) == 0)
        mexErrMsgTxt("sdplr: Input 5 must be of type structure, please.");
      ...
      if(nrhs > 5) {
        ...
        if(mxIsStruct(lrA_) == 0)
          mexErrMsgTxt("sdplr: Input 6 must be of type struct, please.");
        ...
        for(i = 0; i < mxGetM(lrA_)*mxGetN(lrA_); i++) {
          ...
          if(field == NULL || mxGetM(field)*mxGetN(field) != 1 ||
            ...
            (int)mxGetScalar(field) < 0 || (int)mxGetScalar(field) > m)
            mexErrMsgTxt("sdplr: Input 6 has incorrect format.");

          field = mxGetField(lrA_,i,"start");
          if(field == NULL || mxGetM(field)*mxGetN(field) != 1 ||
            mxIsDouble(field) == 0 || mxIsComplex(field) == 1 ||
            (int)mxGetScalar(field) < 0 || (int)mxGetScalar(field) > n_)
            mexErrMsgTxt("sdplr: Input 6 has incorrect format.");
   ...
   */
  MYCALLOC(inblk, int, n_);
  MYCALLOC(inblkptr, int, numblk+2);
  h = 0; k = 1;
  ...
  if(Ks_first == 1) {
    if(Ks_ != NULL) {
      ...
      for(i = 0; i < mymax( mxGetM(Ks_) , mxGetN(Ks_) ); i++) {
        inblkptr[k] = h;
        for(j = 0; j < (int)mat[i]*(int)mat[i]; j++) {
          inblk[h] = k;
          h++;
        }
        k++;
  ...
  if(lrA_ != NULL) {
    ...
    for(h = 0; h < mxGetM(lrA_)*mxGetN(lrA_); h++) {
      ...
      if(A_tr) i = (int)mxGetScalar(field) - 1;
      else     j = (int)mxGetScalar(field) - 1;

      field = mxGetField(lrA_,h,"cons");
      if(A_tr) j = (int)mxGetScalar(field) - 1;
      else     i = (int)mxGetScalar(field) - 1;

      classifyApq(blksz, blktype, inblk, inblkptr, i, j, A_tr, &cons, &blk, &ii, &jj);

      if(ii != 1 || jj != 1)
        mexErrMsgTxt("sdplr: Internal error (6).");
      if(cons != (int)mxGetScalar(field))
        mexErrMsgTxt("sdplr: Internal error (6).");
  ...
  MYCALLOC(CArow, int, nnz);
  MYCALLOC(CAcol, int, nnz);
  ...
  MYCALLOC(tempint, int, (m+1)*numblk);
    ...
    if(c_sp) {
      ...
      for(j = 0; j < mxGetN(c_); j++)
        for(i = jc[j]; i <= jc[j+1]-1; i++) {
          classifyApq(blksz, blktype, inblk, inblkptr, ir[i], j, 1 - c_tr, &cons, &blk, &ii, &jj);
          cons = 0;
          if(ii >= jj) tempint[ DATABLOCKIND(cons,blk,numblk) ]++;
        }
    }
    else{
      for(i = 0; i < n_; i++) {
  ...
  if(lrA_ != NULL) {
    ...
    for(h = 0; h < mxGetM(lrA_)*mxGetN(lrA_); h++) {
      ...
      tempint[ DATABLOCKIND(cons,blk,numblk) ] += k*(blksz[blk-1] + 1);
    ...
    if(c_sp) {
      ...
      for(j = 0; j < mxGetN(c_); j++)
        for(i = jc[j]; i <= jc[j+1]-1; i++) {
          ...
          cons = 0;
          if(ii >= jj) {
            h = CAinfo_entptr[ DATABLOCKIND(cons,blk,numblk) ] + tempint[ DATABLOCKIND(cons,blk,numblk) ];
            CAent[h] = mat[i];
            h = CAinfo_rowcolptr[ DATABLOCKIND(cons,blk,numblk) ] + tempint[ DATABLOCKIND(cons,blk,numblk) ];
            CArow[h] = ii;
            CAcol[h] = jj;
            tempint[ DATABLOCKIND(cons,blk,numblk) ]++;
          }
        }
    }
    else{
  ...
  getparams(NULL, &inputtype, &rho_f, &rho_c, &method,
...
*/
  ...
  if(nrhs > 4) {
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      rankreduce = (int)mxGetScalar(field);

    field = mxGetField(pars_, 0, "limit");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      timelim = (int)mxGetScalar(field);

    field = mxGetField(pars_, 0, "printlevel");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      printlevel = (int)mxGetScalar(field);

    field = mxGetField(pars_, 0, "dthresh_dim");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      dthresh_dim = (int)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      numbfgsvecs = (int)mxGetScalar(field);

    field = mxGetField(pars_, 0, "feasswitch");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      mixedtol = mxGetScalar(field);

    field = mxGetField(pars_, 0, "auxtn");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      doAR = (int)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      checkbd = (int)mxGetScalar(field);

    field = mxGetField(pars_, 0, "bdtype");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      typebd = (int)mxGetScalar(field);

    field = mxGetField(pars_, 0, "writeraw");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      writeraw = (int)mxGetScalar(field);
    else writeraw = 0;

    field = mxGetField(pars_, 0, "soln_factored");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      soln_factored = (int)mxGetScalar(field);
  ...
  if(numbfgsvecs <= 0) {
    mexErrMsgTxt("sdplr: Parameter 'numlbfgs' must be a positive integer.");
  }

  if(mixedtol <= 0.0) {
    mexErrMsgTxt("sdplr: Parameter 'feasswitch' must be positive.");
  }
  ...
  if(forcerank != NULL) {
    ...
    for(k = 0; k < numblk; k++) {
      ...
      if((int)forcerank[k] >= 1 && (int)forcerank[k] <= j)
        maxranks[k] = (int)forcerank[k];
  ...
  if(nlhs > 0) {
    ...
    if(nlhs > 1) {
      ...
      if(nlhs > 2) {
        ...
        plhs[2] = mxCreateStructMatrix(1,1,5,infonames);
        if(plhs[2] == NULL) {
...
int classifyApq(int* blksz, char* blktype, int* inblk, int* inblkptr, int p, int q, int A_tr, int* cons, int* blk, int* i, int* j)
{
  int r, c;
