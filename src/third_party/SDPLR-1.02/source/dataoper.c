    ...
    for(k = 1; k <= data->numblk; k++) {
      ...
      if(data->blktype[k] == SDPBLK) {
        ...
        if(data->XS_blksto[k] == SPARSE) {
          ...
          for(j = 1; j <= blksz; j++)
            for(i = colptr[j]; i <= colptr[j+1]-1; i++) {
              ptr1 = U + base + rowind[i]; ptr2 = U + base + j;
              ptr3 = V + base + rowind[i]; ptr4 = V + base + j;
              ctr = rank;
              tempval = 0.0;
              while(ctr-- > 0) {
                tempval += (*ptr1)*(*ptr4) + (*ptr2)*(*ptr3);
                ptr1 += blksz; ptr2 += blksz; ptr3 += blksz; ptr4 += blksz;
              }
              UVt[i] = 0.5*tempval;
            }
...
int gradient(problemdata* data, double* R)
{
  ...
  StimesR(data, data->S, data->y, R, G, 1);
...
int StimesR(problemdata *data, double *S, double *y, double *R, double *result, int save)
{
  ...
  for(k = 1; k <= data->numblk; k++) {
    Stimesmat(data, S + data->XS_blkptr[k] - 1, y, R + base, result + base, data->blksz[k], data->rank[k], k, save);
...
int Stimesmat(problemdata *data, double *S, double *y, double* vec, double* result, int n, int m, int k, int save)
{
  ...
  if(data->blktype[k] == SDPBLK) {
    ...
    // matrices are already incorporated into S explicitly.
    ...
    if(data->XS_blksto[k] == SPARSE) {
      ...
      for(h = 1; h <= data->nlrind[k]; h++) {
        ...
        // Form VU, put into result scaled by 
        // (What the heck is the if-else logic?!)
        if(i != 0 && save == 1 && ( (data->method == 2 || (data->method == 3 && data->method3_switch == 1)) && data->doAR == 1 )) {
          dgemm_(&transno, &transno, &n, &m, &(A->lr->ncol), y+i,
                 A->lr->ent + 1, &n, U + 1, &(A->lr->ncol), &zero,
                 data->AR[i][k] + 1, &n);
          EASYDAXPY(n*m, 1.0, data->AR[i][k], result);
        }
        else {
          dgemm_(&transno, &transno, &n, &m, &(A->lr->ncol), y+i,
...
int AToper(problemdata* data, double* y, double* S, int obj)
{
  ...
  for(h = 1; h <= data->lr_num; h++) {
    ...
    if(data->XS_blksto[k] == DENSE) {
      ...
      // Setup easy pointer to data matrix
