...
int dorankreduce(problemdata* d, double* R)
{
  int h, i, j, k, p, q, base, base1, base2;
  int maxrank, maxblksz, newrank=0, **save;
  ...
  MYCALLOC(save,  int*,   d->numblk + 1);
  for(k = 1; k <= d->numblk; k++)
    MYCALLOC(save[k], int, d->rank[k] + 1);

  lwork = mymax(1, 3*maxblksz + 1);
  MYCALLOC(work, double, lwork    + 1);
  MYCALLOC(jpvt, int,    maxblksz + 1);
  ...
  for(k = 1; k <= d->numblk; k++) {
    ...
    if(d->blktype[k] == 's') {
      ...
      // Copy permuted U into tempU and do transpose at the same time
      for(j = 1; j <= q; j++)
        for(i = 1; i <= p; i++)
      ...
      if(save[k][0] != 0) {
        ...
        if(save[k][0] == -1) {
          ...
          for(h = 1; h <= d->blksz[k]*(d->maxrank[k] - d->rank[k]); h++) {
            R[base1 + d->blksz[k]*d->rank[k] + h] = (double)(rand() - rand())/RAND_MAX;
