...
void Newton::terminate()
{
  ...
  if (bMat_type == SPARSE){
    ...
    DeleteArray(ordering);
    DeleteArray(reverse_ordering);
                             ...
                             FILE* fpOut)
{
  ...
  case 1: {
    ...
    initialize_sparse_bMat(m, chordal.newToOldIV_MMD,
			   chordal.symbfacIVL_MMD);
  ...
  case 2: {
    ...
    printf("SPARSE computation\n");
    fprintf(fpOut,"SPARSE computation\n");
    initialize_sparse_bMat(m, chordal.newToOldIV_ND,
			   chordal.symbfacIVL_ND);
    make_aggrigateIndex(inputData);
    break;
  }
  case 3: {
    ...
    printf("SPARSE computation\n");
    fprintf(fpOut,"SPARSE computation\n");
    initialize_sparse_bMat(m, chordal.newToOldIV_MS,
			   chordal.symbfacIVL_MS);
    make_aggrigateIndex(inputData);
    break;
  }
  case 4: {
    ...
    printf("SPARSE computation\n");
    fprintf(fpOut,"SPARSE computation\n");
    initialize_sparse_bMat(m, chordal.newToOldIV_NDMS,
			   chordal.symbfacIVL_NDMS);
    make_aggrigateIndex(inputData);
    break;
  }
  }
				...
				double DenseRatio, double Kappa)
{
  ...
  int* upNonZeroCount;
  NewArray(upNonZeroCount,int,m*SDP_nBlock);
  ...
  for (int l=0; l<SDP_nBlock; ++l) {
    ...
    for (int k=0; k < inputData.SDP_nConstraint[l]; k++){
      ...
      double inz = inputData.A[i].SDP_sp_block[ib].NonZeroEffect;

      double f1,f2,f3;
      double n       = inputData.A[i].SDP_sp_block[ib].nRow;
      double up      = upNonZeroCount[i*SDP_nBlock + l];
		    ...
		    bool& hasF2Gcal)
{
  ...
  case SparseMatrix::SPARSE:
    ...
    for (int index = 0; index < Aj.NonZeroCount; ++index) {
      ...
      value2 = ddot_f77(&n, &X.de_ele[alpha], &n,
			&F.de_ele[0+n*beta], &IONE);
      ret += value1*value2;
      if (alpha!=beta) {
	value2 = ddot_f77(&n, &X.de_ele[beta], &n,
...
void Newton::display_index(FILE* fpout)
{
  ...
  for (int l=0; l<SDP_nBlock; l++){
    ...
    for (int k=0; k<SDP_number[l]; k++){
      printf("cons1:%d const2:%d block1:%d block2:%d sp_bMat:%d \n",
	     SDP_constraint1[l][k],SDP_constraint2[l][k],
	     SDP_blockIndex1[l][k],SDP_blockIndex2[l][k], 
  ...
  for (int l=0; l<SOCP_nBlock; l++){
    ...
    for (int k=0; k<SOCP_number[l]; k++){
      printf("cons1:%d const2:%d block1:%d block2:%d sp_bMat:%d \n",
	     SOCP_constraint1[l][k],SOCP_constraint2[l][k],
	     SOCP_blockIndex1[l][k],SOCP_blockIndex2[l][k], 
  ...
  for (int l=0; l<LP_nBlock; l++){
    ...
    for (int k=0; k<LP_number[l]; k++){
      printf("cons1:%d const2:%d block1:%d block2:%d sp_bMat:%d \n",
	     LP_constraint1[l][k],LP_constraint2[l][k],
	     LP_blockIndex1[l][k],LP_blockIndex2[l][k], 
